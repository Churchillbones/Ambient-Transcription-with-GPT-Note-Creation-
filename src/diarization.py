...
async def generate_gpt_speaker_tags(transcript: str, api_key: Optional[str]) -> str:
    """Use Azure OpenAI (GPT) to generate more accurate speaker tags."""
    if not api_key:
    ...
    try:
        ...
        logger.info(f"Requesting speaker tagging from Azure OpenAI model: {config['MODEL_NAME']}")
